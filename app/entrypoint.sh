...
if [ "$1" = 'mattermost' ]; then
  ...
  if [ ! -f "$MM_CONFIG" ]; then
    ...
    jq --argjson v $MM_TEAMSETTINGS_MAXUSERSPERTEAM '.TeamSettings.MaxUsersPerTeam = $v' "$MM_CONFIG" >"$MM_CONFIG.tmp" && mv "$MM_CONFIG.tmp" "$MM_CONFIG"
    ...
    jq --argjson v $MM_LOGSETTINGS_ENABLECONSOLE '.LogSettings.EnableConsole = $v' "$MM_CONFIG" >"$MM_CONFIG.tmp" && mv "$MM_CONFIG.tmp" "$MM_CONFIG"
    ...
    jq --argjson v $MM_FILESETTINGS_ENABLEPUBLICLINK '.FileSettings.EnablePublicLink = $v' "$MM_CONFIG" >"$MM_CONFIG.tmp" && mv "$MM_CONFIG.tmp" "$MM_CONFIG"
    jq --arg v $MM_FILESETTINGS_PUBLICSALT '.FileSettings.PublicLinkSalt = $v' "$MM_CONFIG" >"$MM_CONFIG.tmp" && mv "$MM_CONFIG.tmp" "$MM_CONFIG"
    jq --argjson v $MM_EMAILSETTINGS_SENDEMAILNOTIFICATIONS '.EmailSettings.SendEmailNotifications = $v' "$MM_CONFIG" >"$MM_CONFIG.tmp" && mv "$MM_CONFIG.tmp" "$MM_CONFIG"
    jq --argjson v $MM_EMAILSETTINGS_REQUIREEMAILVERIFICATION '.EmailSettings.RequireEmailVerification = $v' "$MM_CONFIG" >"$MM_CONFIG.tmp" && mv "$MM_CONFIG.tmp" "$MM_CONFIG"
    ...
    jq --argjson v $MM_EMAILSETTINGS_ENABLESMTPAUTH '.EmailSettings.EnableSMTPAuth = $v' "$MM_CONFIG" >"$MM_CONFIG.tmp" && mv "$MM_CONFIG.tmp" "$MM_CONFIG"
    ...
    jq --argjson v $MM_RATELIMITSETTINGS_ENABLE '.RateLimitSettings.Enable = $v' "$MM_CONFIG" >"$MM_CONFIG.tmp" && mv "$MM_CONFIG.tmp" "$MM_CONFIG"
